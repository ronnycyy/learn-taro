...
/**
 * 用户组件 转  wx组件
 * @param {*} ComponentClass 用户写的类，类似于React里的类组件
 * @param {*} isPage 默认是页面
 */
function createComponent(ComponentClass, isPage) {
  ...
  try {
    ...
    Current.index = 0
    // 组件的state 就是 _createData() 页面要使用的数据  { name: xx, color: xx }
  ...
  const weappComponentConf = {
    ...
    ready() {
      // 小程序 逻辑层基础库 调 ready 方法，执行 Taro 生命周期 componentDidMount
      // 其实就是做了代理....
  ...
  // 一大堆生命周期来袭:  componentDidMount/componentDidShow/componentDidHide...
  if (isPage) {
    // wx Page?
    weappComponentConf.methods = weappComponentConf.methods || {}
